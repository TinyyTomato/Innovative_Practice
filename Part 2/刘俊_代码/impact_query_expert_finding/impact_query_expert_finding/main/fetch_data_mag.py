...
import impact_query_expert_finding.evaluation.visual_LExR
...
def run(parameters):
    ...
    data_folder = os.path.join(parameters["output_dir"], "VT")
    ...
    # text_file = os.path.join(data_folder, config[""])
    ...
    full_stats_path = os.path.join(parameters["dump_dir"], "dataset_vT")
    ...
    dataset = impact_query_expert_finding.data.sets.DataSet("mag",
                                                            "/ddisk/lj/DBLP/data/V4/paper.json",
                                                            "/ddisk/lj/LExR/data/V4/author.json",
                                                            version="V4")
    ...
    dataset.save_LExR(full_path)
    #
    print("Creating cleaned dataset")
    cleaned_path = os.path.join(data_folder, "dataset_cleaned")
    cleaned_stats_path = os.path.join(parameters["dump_dir"], "dataset_vT_stats_cleaned")
    ...
    dataset.clean_associations_LExR(max_documents_per_candidates=100, min_documents_per_candidates=1)
    impact_query_expert_finding.evaluation.visual_LExR.plot_stats(dataset,
                                                                  cleaned_stats_path,
                                                                  min_documents=10,
                                                                  min_in_citations=10,
                                                                  min_out_citations=10
                                                                  )
    dataset.save_LExR(cleaned_path)
    #
    ...
    associations_stats_path = os.path.join(parameters["dump_dir"], "dataset_vT_stats_associations")
    ...
    dataset.clean_associations_LExR(max_documents_per_candidates=100, min_documents_per_candidates=2)
    documents_set, candidates_set = impact_query_expert_finding.tools.graphs.extract_experts_associations_subgraph(
        dataset,
        length_walks=5,
        number_of_walks=50
    )
    dataset.reduce_LExR(documents_set, candidates_set)
    impact_query_expert_finding.evaluation.visual_LExR.plot_stats(dataset,
    ...
    dataset.save_LExR(associations_path)
