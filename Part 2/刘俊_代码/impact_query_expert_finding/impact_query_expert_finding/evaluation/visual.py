...
def record_evaluation(eval, prefix, hybird=None, parameters=None):
    # eval, prefix = "no_prefix", path_visuals = None
    # + str(int(parameters["para"] * 10)) + ":" + str(int((1 - parameters['para']) * 10))
    # title = " - ".join([i + ": " + str(j) for i, j in eval["info"].items()])
    title = parameters['data_type'] + ":/" + parameters['query_type'] + "->" + parameters['rank_type'] + "->"
    title += " - ".join([i + "={0:.3f}".format(j) for i, j in eval["metrics"].items()])
