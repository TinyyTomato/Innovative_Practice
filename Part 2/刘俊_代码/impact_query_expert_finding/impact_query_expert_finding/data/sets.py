...
class DataSet:
    def __init__(self, name, documents_file_path=None, experts_folder_path=None, version=None, type=None):
        ...
        if documents_file_path is not None and experts_folder_path is not None:
            if name == "aminer":
                self.getAminer(documents_file_path, experts_folder_path, self.version, type)
            elif name == "acm":
    ...
    def save(self, dir_path):
        ...
        impact_query_expert_finding.data.io.save_as_json(dir_path, "df_venues", self.ds.venues)
    ...
    def save_LExR(self, dir_path):
        ...
        impact_query_expert_finding.data.io.save_as_json(dir_path, "df_candidates", self.ds.candidates)
        scipy.sparse.save_npz(os.path.join(dir_path, "df_associations"), self.ds.associations)
        scipy.sparse.save_npz(os.path.join(dir_path, "df_venues"), self.ds.venues)
        scipy.sparse.save_npz(os.path.join(dir_path, "df_citations"), self.ds.citations)
        impact_query_expert_finding.data.io.save_as_json(dir_path, "gt_topics", self.gt.topics)
        impact_query_expert_finding.data.io.save_as_json(dir_path, "gt_candidates", self.gt.candidates)
        impact_query_expert_finding.data.io.save_as_json(dir_path, "gt_experts_mask", self.gt.experts_mask.tolist())
    ...
    def load(self, dir_path):
        ...
        # self.ds.venues = impact_query_expert_finding.data.io.load_as_json(dir_path, "df_venues")
        ...
        self.gt.experts_mask = np.array(impact_query_expert_finding.data.io.load_as_json(dir_path, "gt_experts_mask"),
                                        dtype=np.int_)
    ...
    def getAminer(self, documents_file_path, experts_folder_path, version, type=None):
        # Load data from disk
    ...
    def getAcm(self, documents_file_path, experts_folder_path, version):
        ...
        documents = impact_query_expert_finding.data.io_aminer.load_papers(documents_file_path)
        print("Loading experts from aminer dataset...")
        experts = impact_query_expert_finding.data.io_aminer.load_experts(experts_folder_path)
        ...
        for i, d in enumerate(documents):
            ...
            for i in range(1, n + 1):
                dem = dem + (1.0 / i)
            for j, c in enumerate(d['authors']):
                dat.append(self.author_weight(j, dem))
                row_ind.append(i)
                col_ind.append(candidates_index_dict[c.strip()])
        self.ds.associations = scipy.sparse.csr_matrix((np.array(dat), (np.array(row_ind), np.array(col_ind))),
                                                       shape=(M, N))
        ...
        # create list of venues
        for d in documents:
            self.ds.venues.append(d['venues'])
        venues_set = set(self.ds.venues)
        self.ds.venues = lint(venues_set)
        M = len(self.ds.documents)
        N = len(self.ds.venues)
        ...
        for i, d in enumerate(documents):
            for j, c in enumerate(d['venues']):
                dat.append(1)
                row_ind.append(i)
                col_ind.append(venues_index_dict[c])

        self.ds.venues = scipy.sparse.csr_matrix((np.array(dat), (np.array(row_ind), np.array(col_ind))),
                                                 shape=(M, N))
        self.print_stats()
