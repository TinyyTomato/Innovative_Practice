...
class Model:
    ...
    def fit(self,
        ...
        random_walker = impact_query_expert_finding.preprocessing.graph.random_walker.RandomWalker(
            X,
            walks_length=40,
            walks_number=10
        )
        random_walks = random_walker.build_random_walks()
        slider = impact_query_expert_finding.preprocessing.graph.window_slider.WindowSlider(
            random_walks,
            X.shape[0],
            window_size=5,
        ...
        self.vocab = impact_query_expert_finding.preprocessing.text.dictionary.Dictionary(M)
        logger.debug("Building tf vectors")
        self.M = impact_query_expert_finding.preprocessing.text.vectorizers.get_tf_dictionary(self.vocab)
