...
class PanopticModel:
    ...
    def fit(self, x, Y, dataset=None, mask=None):
        ...
        for i in range(self.dataset.ds.associations.shape[1]):
            self.authors_metadocs.append(
                " ".join([self.dataset.ds.documents[j] for j in self.dataset.ds.associations[:, i].nonzero()[0]]))
    ...
    def predict(self, query, leave_one_out=None):
        ...
        if leave_one_out is not None:
            ...
            for c in candiates_to_recompute:
                metadocs = " ".join(
                    [self.dataset.ds.documents[j] for j in self.dataset.ds.associations[:, c].nonzero()[0] if c != j])
