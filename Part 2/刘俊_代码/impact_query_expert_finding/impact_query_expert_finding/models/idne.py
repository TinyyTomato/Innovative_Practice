...
import impact_query_expert_finding.models.data_generator
import impact_query_expert_finding.models.tools
from impact_query_expert_finding.models.transformer import activation_attention, cosine_loss

warnings.filterwarnings('ignore')
import matplotlib.pyplot as plt
import scipy.spatial.distance
import impact_query_expert_finding.preprocessing.graph.random_walker
import impact_query_expert_finding.preprocessing.graph.window_slider
...
class Model:
    ...
    def __data_generator(self, X, M, number_negative, number_iterations, batch_size):
        for i, (u, v, x) in enumerate(impact_query_expert_finding.models.data_generator.async_batches(
                X,
                number_negative,
                number_iterations,
                batch_size
    ...
    def fit(self, X, M, vocab=None):
        ...
        if self.vocab is None:
            self.vocab = impact_query_expert_finding.preprocessing.text.dictionary.Dictionary(M, min_df=5, max_df_ratio=0.25)
        ...
        clock = impact_query_expert_finding.models.tools.Clock(
